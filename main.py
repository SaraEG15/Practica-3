...
class GrammarApp(QMainWindow):
    def __init__(self):
        ...
        # Etiqueta e input para la expresión
        ...
        # Botón para derivación por izquierda
        ...
        # Botón para derivación por derecha
    ...
    def build_ast(self, tree):
        ...
        if isinstance(tree, Tree):
            if tree.label() in {'E', 'T', 'F'}:
                if len(tree) == 1:
                    return self.build_ast(tree[0])  # Descartar nodos innecesarios
                else:
                    return Tree(tree[1], [self.build_ast(tree[0]), self.build_ast(tree[2])])  # Operación binaria
            elif tree.label() in {'VAR', 'NUM'}:
                return Tree(tree.label(), [tree[0]])  # Nodo hoja
            else:
